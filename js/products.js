...
if (numberOfProductContainer !== null) {
  numberOfProducts(4);
}
...
function createHTML(product) {
  number++;
  isEmpty = false;
  productContainer.innerHTML += `<a href="products/product.html" data-product=${product.id}>
  ...
  sort.addEventListener("change", (event) => {
    ...
    form.addEventListener("submit", function (event) {
      ...
      currentArray.forEach(function (item) {
        if (
          sizeChoice.value === "size" &&
          colorChoice.value === "Color" &&
          priceChoice.value === "prices"
        ) {
          createHTML(item);
          ...
          for (let i = 0; i < item.size.length; i++) {
            if (item.size[i] === sizeChoice.value) {
              ...
              if (
                ...
                colorChoice.value === "Color" &&
                priceChoice.value === productPrice
              ) {
                createHTML(item);
              } else if (
                colorChoice.value === item.color &&
                priceChoice.value === "prices"
              ) {
                createHTML(item);
              } else if (
